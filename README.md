...
네이버 검색 화면을 읽는 API 키 없는 별도 모듈은
[네이버 검색 크롤러 사용법](jobs/naver_search/README.md)을 참고하세요.
검색어·장소명·주소를 바꿔 재사용할 수 있으며 실제 접근 제한 및 검증 범위가 문서에 명시되어 있습니다.

## 네이버지도 블로그 리뷰 CSV 작업

`jobs/naver_map/crawl_script.py`에 장소별 블로그 리뷰 메타데이터 수집 작업을 추가했습니다.
기본값은 아소토베이커리 예시 ID `1141254769`, 최대 50개이며 URL·제목·작성자·작성일·요약을 CSV로 저장합니다.
실사이트는 미검증이며 네이버의 사전 허락과 robots 허용이 필요합니다.
[실행 방법·오프라인 드라이런·검증 범위](jobs/naver_map/README.md)를 먼저 확인하세요.
